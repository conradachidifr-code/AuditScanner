...
WORKSHOP_NOTES: dict[str, str] = {
    ...
    "INC-02": "Verify RACI for cloud incidents: CCoE vs SOC vs metiers vs RSSI.",
    "INC-04": "Verify incident severity matrix exists for cloud incidents.",
    "INC-05": "Verify playbooks exist for: compromised account, data breach, DDoS, ransomware. Check DEX or SOC space.",
    ...
    "INC-09": "Verify forensics procedure: evidence preservation, chain of custody.",
    ...
    "INC-12": "Known: AWS Support managed by separate EDF group. Verify escalation process.",
    "INC-13": "Verify tabletop or live exercises have been performed.",
    "INC-14": "Verify RETEX exists for August 2023 SSH incident. Check FEX documentation.",
}
...
def _inc03(account_id: str, account_name: str, region: str, ctx: CheckContext) -> AuditResult:
    ...
    evidence = {
        ...
        "incident_rule_count": collection_count(guardduty_rules + cloudtrail_rules + config_rules),
    }
    if guardduty_rules or cloudtrail_rules or config_rules:
        return ctx.results.audit_result(
            account_id, account_name, region, "INC-03", "PASS", evidence,
            "EventBridge rules found for GuardDuty, CloudTrail, or Config incident events",
        )
    return ctx.results.audit_result(
...
def _inc06(account_id: str, account_name: str, region: str, ctx: CheckContext) -> AuditResult:
    ...
    if quarantine_ous:
        return ctx.results.audit_result(
            ...
            "Quarantine OU found in organization structure",
        )
    return ctx.results.audit_result(
...
def _inc15(account_id: str, account_name: str, region: str, ctx: CheckContext) -> AuditResult:
    ...
    if has_property(data, "Events"):
        ...
        for event in events:
            ...
            if re.search(r"BreakGlass|Emergency|CCOScriptAdmin", event_text):
                break_glass_count += 1
    ...
    if break_glass_count > 0:
        return ctx.results.audit_result(
            ...
            "Break-glass AssumeRole activity logged in CloudTrail",
        )
    if cloudtrail_active:
        return ctx.results.audit_result(
            account_id, account_name, region, "INC-15", "PARTIAL", evidence,
            "CloudTrail active but no break-glass AssumeRole events found in sample",
        )
    return ctx.results.audit_result(
